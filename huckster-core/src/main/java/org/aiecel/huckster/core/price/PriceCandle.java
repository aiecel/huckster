...
@AllArgsConstructor
@EqualsAndHashCode
public class PriceCandle implements OHLC<BigDecimal>, Priced {
    ...
    public PriceCandle(Instant openTimestamp, Timeframe timeframe, BigDecimal initialPrice) {
        this(openTimestamp, timeframe, initialPrice, initialPrice, initialPrice, initialPrice);
    }

    public static PriceCandle fromOHLC(OHLC<BigDecimal> ohlc) {
        return new PriceCandle(ohlc.getTimestamp(),
                ohlc.timeframe(),
                ohlc.open(),
                ohlc.high(),
                ohlc.low(),
